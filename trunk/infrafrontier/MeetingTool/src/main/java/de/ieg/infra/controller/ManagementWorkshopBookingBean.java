...
public class ManagementWorkshopBookingBean extends BookingBean {

	private ManagementWorkshopBooking booking = new ManagementWorkshopBooking();
	 ...
	 * @return
	 */
	public ManagementWorkshopBooking getBooking() {
		return booking;
	 ...
	 * @param booking
	 */
	public void setBooking(ManagementWorkshopBooking booking) {
	...
	public String generateEmailConfirmationText() {
		...
		text.append(booking.getArrivalTime());

		text.append("\r\n");

		text.append("Departure time:      ");

		text.append(booking.getDepatureTime());
		...
		text.append(booking.getNumberOfNights() != null ? booking
				.getNumberOfNights().toString() : null);
	...
	public String generateBookingText() {
		StringBuffer text = new StringBuffer("Bookings for " + event.getName()
				+ "so far (copy and paste lines below to Excel\r\n\r\n");
	...
	public void bookEvent(ActionEvent ev) {
		try {
			...
			Utils.addInfoMessage(
					"bookingForm",
					"Your booking was submitted. A confirmation message will be sent to your email address.");

			booking = new ManagementWorkshopBooking();
		} catch (Exception ex) {
			Logger.getLogger(BookingBean.class).error(
					"bookEvent() creates an error: " + ex.getMessage());

			Utils.addErrorMessage("bookingForm:bookGrid",
					"Error! Booking failed. Please contact the INFRAFRONTIER Project Office");
